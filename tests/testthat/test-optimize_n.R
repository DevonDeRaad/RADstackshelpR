...
test_that("optimize_n generates output of the appropriate class (list)", {
  ...
  #opt.m<-optimize_n(nequalsMminus1 = "~/Desktop/RADstackshelpR/inst/extdata/populations.snps.vcf.gz")
  #find data in package using CRAN friendly syntax
  opt.m<- optimize_n(nequalsMminus1 = system.file("extdata", "populations.snps.vcf.gz", package = "RADstackshelpR"))
...
test_that("optimize_n generates dataframes with appropriate dimensions when all slots are filled", {
  #find data in package using CRAN friendly syntax
  opt.m<- optimize_n(nequalsMminus1 = system.file("extdata", "populations.snps.vcf.gz", package = "RADstackshelpR"),
                        nequalsM = system.file("extdata", "populations.snps.vcf.gz", package = "RADstackshelpR"),
                        nequalsMplus1 = system.file("extdata", "populations.snps.vcf.gz", package = "RADstackshelpR"))
