...
```
We can see that there is a group of samples with an excess of missing data, which clump at the bottom of the 'SNPs retained by filtering scheme' plot, and are outliers in the dotplot showing missing data proportion per sample. We want to remove those samples which we won't be able to make inferences about due to excess missing data.
```{r, , fig.height = 8, fig.width = 10, fig.align = "center"}
...
Plot heatmaps showing the depth and genotype quality. Each row is a SNP and each column is a sample.
```{r, fig.height = 8, fig.width = 10, fig.align = "center"}
...
Overall, the GQ and DP are consistenly high across SNPs and samples. We could remove a few more of the samples (columns) with the lowest depths and most missing data if we wanted to clean up our dataset even more.

Now we can use the convenient function 'write.vcf' from the package vcfR to export our filtered vcf file for downstream analyses
```{r}
...
vcftools --vcf filtered.vcf --thin 10000 --recode --out vcf
