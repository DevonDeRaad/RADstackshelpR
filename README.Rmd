...
If you are using RADStackshelpR in your pipeline for RAD analyses, I recommend citing both RADStackshelpR and the R package vcfR (which is used heavily inside of RADStackshelpR functions) e.g., "We used the R packages *RADStackhelpR* (DeRaad, 2021) and *vcfR* (Knaus and Grunwald, 2017) to calculate summary statistics from *Stacks* output vcf files and determine the optimal parameters for assembling RAD loci de novo."
...
To see full documentation of all functions and three detailed vignettes illustrating use cases, please check out the [pkgdown site](https://devonderaad.github.io/RADstackshelpR/index.html) for RADstackshelpR. For a quick start, simply follow the directions below:
...
```{r, dpi=150, out.width="65%",out.height="65%", fig.align = "center", fig.width = 3.5, fig.height = 2.5}
...
```{r, dpi=150, out.width="100%",out.height="100%", fig.align = "center", fig.width = 6, fig.height = 4}
